...
import StatistikIcon from '../ui/Icons/StatistikIcon';
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="hidden lg:flex flex-col flex-grow space-y-6 px-1 w-full">
          {navItems.map((item) => {
            ...
            return (
              <div key={item.id} className="relative">
                ...
                >
                  ...
                  {isPlaceholder ? (
                    <div className="h-7 w-full rounded-md bg-sidebar-placeholder opacity-0 lg:group-hover:opacity-100 transition-opacity duration-200 delay-100 flex-shrink-0" />
                  ) : (
                    <span className="text-md font-medium whitespace-nowrap opacity-0 lg:group-hover:opacity-100 transition-opacity duration-200 delay-100">
                      {item.name}
                    </span>
                  )}
                </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed top-16 left-0 w-full bg-sidebar-bg/95 backdrop-blur-md shadow-2xl flex flex-col lg:hidden z-40 border-t border-white/10 animate-fade-in pb-4 pt-3 px-3 space-y-2">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = currentRoute === item.id;
